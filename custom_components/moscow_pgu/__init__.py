...
CONFIG_SCHEMA = vol.Schema({
    DOMAIN: vol.All(cv.ensure_list, vol.Length(min=1), [{
        ...
        vol.Optional(CONF_SCAN_INTERVAL): vol.Any(
            ...
            vol.Schema({
                vol.Optional(CONF_WATER_COUNTERS): cv.positive_time_period,
                vol.Optional(CONF_FSSP_DEBTS): cv.positive_time_period,
                vol.Optional(CONF_PROFILE): cv.positive_time_period,
...
async def async_setup_entry(hass: HomeAssistantType, config_entry: ConfigEntry) -> bool:
    ...
    if config_entry.source == SOURCE_IMPORT:
        ...
        device_config = data.get(CONF_DEVICE_INFO, {})

    else:
        data = config_entry.data
        device_config = (config_entry.options or {}).get(CONF_DEVICE_INFO, {})
